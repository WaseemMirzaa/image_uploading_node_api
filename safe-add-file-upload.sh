...
if [ ! -f "server.js" ]; then
    echo "❌ server.js not found. Please run this script in your project directory."
    exit 1
fi
...
if ! grep -q "// File upload configuration" server.js; then
...
EOF
    ...
    # Use awk to safely insert the configuration
    cp server.js server.js.tmp
    awk '/require\(.*dotenv.*\)\.config\(\);/ { print; while ((getline line < "temp_config.js") > 0) print line; close("temp_config.js"); next } 1' server.js.tmp > server.js
    rm server.js.tmp temp_config.js
fi

# Add static file serving for uploaded files (if not exists)
if ! grep -q "app.use('/files'" server.js; then
    echo "Adding static file serving..."
    cp server.js server.js.tmp
    awk '/app\.use\(express\.urlencoded/ { print; print "app.use('\''/files'\'', express.static(uploadPath));"; next } 1' server.js.tmp > server.js
    rm server.js.tmp
fi

# Add file upload endpoints (if not exists)
if ! grep -q "POST /upload" server.js; then
    echo "Adding file upload endpoints..."
    
    cat >> temp_endpoints.js << 'EOF'
...
EOF
    ...
    # Find a good place to insert the endpoints (before the 404 handler or at the end)
    cp server.js server.js.tmp

    if grep -q "// 404 handler" server.js; then
        # Insert before 404 handler
        awk '/\/\/ 404 handler/ { while ((getline line < "temp_endpoints.js") > 0) print line; close("temp_endpoints.js"); print; next } 1' server.js.tmp > server.js
    else
        # Insert before app.listen
        awk '/app\.listen/ { while ((getline line < "temp_endpoints.js") > 0) print line; close("temp_endpoints.js"); print; next } 1' server.js.tmp > server.js
    fi

    rm server.js.tmp temp_endpoints.js
fi
...
pm2 restart 4secrets-wedding-api || pm2 start server.js --name "4secrets-wedding-api"
